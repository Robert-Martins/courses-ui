...
public final class Functions {
    ...
    private static final Integer MAX_UNTIL_ATTEMPTS = 10;
    ...
    /**
     *
     * Throws an Exception if a Value is Empty
     *
     * @param value Value to be checked if it is Empty
     * @param supplier Supplier Function that throws an exception if value is present
     *
     */
    public static <T, X extends Throwable> void ifEmptyThrows(T value, Supplier<? extends X> supplier) throws X {
        if(Optional.ofNullable(value).isEmpty())
            throw supplier.get();
    }
    ...
    /**
     *
     * Throws an Exception if a Value is Present
     *
     * @param value Value to be checked if it is Empty
     * @param supplier Supplier Function that throws an exception if value is present
     *
     */
    public static <T, U extends Exception> void ifNotEmptyThrow(T value, Supplier<? extends U> supplier) throws U {
    ...
    public static <T, U extends Throwable> void anyRepeatedItemsThrows(Collection<T> collection, Supplier<? extends U> supplier) throws U {
        if (collection.stream().distinct().count() != collection.size())
            throw supplier.get();
    }

    public static <T, U extends Throwable> void anyItemsOnAnotherCollectionThrows(Collection<T> collection, Collection<T> anotherCollection, Supplier<? extends U> supplier) throws U {
        if (collection.stream().anyMatch(anotherCollection::contains))
            throw supplier.get();
    }

    public static <X extends Throwable> void until(Supplier<Boolean> supplier, EmptyAction action, Supplier<? extends X> exceptionSupplier) throws X {
        ...
        do {
            ...
            bool = supplier.get();
            attempts++;
        } while(bool && attempts < MAX_UNTIL_ATTEMPTS);
        if (attempts >= MAX_UNTIL_ATTEMPTS) {
